...
!Obtain path
    ...
    titlenfname = len(titratepart)
    do while (titratepart(titlenfname:titlenfname) .eq. ' ')
        titlenfname = titlenfname - 1
    enddo
    ...
    do
        ...
        if ((c1(1:3) .ne. 'TER') .and. (c1(1:3) .ne. 'END') .and. (MEOF .eq. 0)) then
    ...
    nremark=0
    open(102,file=pathname(1:lenpath)//titratepart(1:titlenfname)//".pqr")
    do
        READ(102,*) fhead
        if (fhead(1:6)=='REMARK') then
            nremark=nremark+1
...
!    print *,nchr,natm
!    nchr=natm
    allocate(atmpos(3,nchr),atmrad(nchr),atmchr(nchr),chrpos(3,nchr),STAT=ierr)
    IF (ierr .NE. 0) THEN
