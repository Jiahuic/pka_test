...
READ(101,*,IOSTAT = MEOF) fhead, titratepart
...
!test only
!do i=1,10
!    write(*,*) i,real(bvct(i)),real(bvct(nface+i)),real(bvct(nface-i)),real(bvct(2*nface-i))
!enddo
...
soleng=0.d0; soleng_exa=0.d0; couleng=0.d0

do iatm=1,nchr
    r0=chrpos(:,iatm)
    if (imd==1 .and. iflag_steric_clashes==0) then
...
!        if (dist == 0.D0) then
!          print *,iatm,jatm,dist,1/eps0/dist*atmchr(iatm)*atmchr(jatm)*para
!        endif
!    enddo
enddo
...
open(101,file="potential.txt", position="append")
write(101,*)  titratepart(1:titlenfname),soleng
close(101)
...
!call output_potential           !output potential at vertices
...
do j=1,nface ! for each triangle
    ...
    H1=G1-eps*G2
    H2=G0-Gk
    if (ichg .le. nchr/2) then
        if (jface .le. nface/2) then
            ptl=ptl+tr_area(j)*H1*xvct(j)
            ptl=ptl+tr_area(j)*H2*xvct(nface+j)
        else
